...
Let us consider a FOA signal $x_m(t)$, with $m = 0 \ldots, M-1$ as the 
%ACN 
channel number, and $M=4$.
Let us further assume that $x_m(t)$ represents the signal captured by an ideal spherical microphone array located in a reverberant enclosure, where a static sound source $s(t)$ is present. The ambisonic room impulse response between source and receiver is represented by $h_m(t)$. In the absence of noise, the recorded signal is the convolutive mix of the source signal $s(t)$ and the RIR, as described in Eq.~\ref{eq:convolutivemixture}:
\begin{equation}
...
Let us consider the zeroth order channel of the recorded signal, $x_0(t)$, and its Short-Time Frequency Transform (STFT) counterpart $X_0(k,n)$, where $k$ and $n$ indicate frequency bin and time frame indices respectively. 
...
In other words, the captured signal is split between a \textit{direct} part $D_m(k, n)$, containing the direct path and the early reflections, and a \textit{reverberant} part $R_m(k, n)$, which mainly contains the diffuse part of the reverberation.
